...
def test_serve_declares_service():
    assert _load_serve().SERVICES == {"hh": "hh_mcp.server"}
...
def test_manifest_env_matches_server_env_map():
    from hh_mcp.server import HH_CONFIG
    env = _manifest()["server"]["mcp_config"]["env"]
    assert set(env) == set(HH_CONFIG.env_map.values())
