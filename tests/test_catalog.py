...
CATALOG = ROOT / "hh_mcp" / "endpoints.yaml"
SVC = "hh"
...
def test_entities_file_ships_with_the_catalog():
    """Карта сущностей это половина поиска и весь инструмент hh_map. Пока
    файла не было, map молча отдавал пустоту во всех пяти серверах."""
    ents = yaml.safe_load((ROOT / "hh_mcp" / "entities.yaml").read_text(encoding="utf-8"))
...
def test_live_questions_hit_the_right_method():
    ...
    idx = EntityIndex.load(ROOT / "hh_mcp" / "entities.yaml")
    catalog = Catalog.from_yaml(CATALOG, entities=idx)
    for question, expected in [('поиск вакансий', 'hh_get_vacancies'), ('мои отклики', 'hh_get_negotiations'), ('опубликовать вакансию', 'hh_publish_vacancy')]:
