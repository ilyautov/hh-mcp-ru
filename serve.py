#!/usr/bin/env python3
"""Запускалка hh-mcp-ru, поднимающая себе окружение сама.
...
текущему процессу и стартует сервер. Дальше запуск мгновенный.
    ...
    python3 serve.py hh               # запустить сервер
    python3 serve.py hh --selfcheck   # проверить установку и выйти

Нужна именно она, а не `uvx hh-mcp-ru`: бандл .mcpb для Claude Desktop
...
def _log(msg: str) -> None:
    print(f"[hh-mcp-ru] {msg}", file=sys.stderr, flush=True)
...
def main() -> None:
    ...
    sys.path.insert(0, str(HERE))  # чтобы импортировался hh_mcp/ из бандла
